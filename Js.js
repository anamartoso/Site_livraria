...
$.ajax(url)
    .done(function(xml){
        $(xml).find("pirata").each(function(){
            $("#cards").append('<div class="card"><a href="individual.html?id='+$(this).find("id").text()+'"><p class="livros">livros </p> <img class="foto"src="'+ $(this).find("id").text() +'.jpg"><p class="vivo">disponivel ou esgotado</p> <p class="nome">'+ $(this).find("nome").text() +'</p> <p class="rec"> <img class="berrys" src="Berrys.png">'+ $(this).find("recompensa").text()+' </p><p> Status:'+ $(this).find("status").text()+'</a> </p><\div>');
        });
    })
    .fail(function(){
